...
function generateHeroSVG(theme, data) {
  ...
  PHRASES.forEach((p, i) => {
    ...
    const x = CX - w / 2;
    clips += `<clipPath id="h-clip${i}"><rect class="h-tw" x="${x.toFixed(1)}" y="86" width="${w.toFixed(1)}" height="24" style="animation-delay:${(i * 3.4).toFixed(1)}s"/></clipPath>`;
    phrases += `<text class="h-ph" x="${CX}" y="103" text-anchor="middle" clip-path="url(#h-clip${i})" style="animation-delay:${(i * 3.4).toFixed(1)}s">${text.replace(/&/g, '&amp;')}</text>`;
  });
  ...
  chips.forEach((c, i) => {
    ...
    chipsSVG += `<g class="chip" style="animation-delay:${800 + i * 150}ms">
      <circle cx="${cx - 58}" cy="146" r="3" fill="${t.accent}"/>
  ...
  <style>
    ...
    @media (prefers-reduced-motion: reduce) {
      .fade, .chip, .h-ph { animation: none; opacity: 1; }
